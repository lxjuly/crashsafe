...
def test_retry_after_blocks_same_tool_across_runs_and_restart(settings: object) -> None:
    store = SQLiteStorage(settings.engine_db)  # type: ignore[attr-defined]
    for customer in ("first", "second"):
        store.create_workflow_run(paid_workflow_definition(customer))
...
def test_non_429_retry_does_not_create_shared_throttle(settings: object) -> None:
    ...
    store = SQLiteStorage(settings.engine_db)  # type: ignore[attr-defined]
    store.create_workflow_run(paid_workflow_definition("one"))
...
def test_longer_deadline_extends_throttle_and_shorter_cannot_reduce_it(
    ...
    store = SQLiteStorage(settings.engine_db)  # type: ignore[attr-defined]
    first = store.create_workflow_run(paid_workflow_definition("one"))
    second = store.create_workflow_run(paid_workflow_definition("two"))
    long_deadline = utc_now() + timedelta(seconds=30)
    short_deadline = utc_now() + timedelta(seconds=10)

    store.record_attempt(first.run_id, first.steps[0].id)
    store.schedule_retry(first.run_id, first.steps[0].id, "long", long_deadline, throttle_tool=True)
    store.record_attempt(second.run_id, second.steps[0].id)
...
def test_throttle_does_not_block_a_different_tool(settings: object) -> None:
    store = SQLiteStorage(settings.engine_db)  # type: ignore[attr-defined]
    blocked = store.create_workflow_run(paid_workflow_definition("one"))
    other = store.create_workflow_run(
        paid_workflow_definition("two"),
        tool_key="other-tool",
    )
    store.record_attempt(blocked.run_id, blocked.steps[0].id)
    store.schedule_retry(
        blocked.run_id,
        blocked.steps[0].id,
        "blocked",
    ...
    assert claim.step.run_id == other.run_id
