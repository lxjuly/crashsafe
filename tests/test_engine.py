...
def test_committed_retry_time_does_not_change_with_configuration(settings: object) -> None:
    ...
    engine.run_once()
    selected_time = store.get_workflow_run(run.run_id).steps[0].next_attempt_at
    ...
    persisted = reopened.get_workflow_run(run.run_id).steps[0].next_attempt_at
    retry_event = reopened.list_events(run.run_id)[-1]
